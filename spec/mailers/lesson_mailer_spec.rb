...
RSpec.describe LessonMailer, type: :mailer do
  describe 'send_lesson' do
    ...
    let(:mail) { described_class.lesson_email(user: user, course: course, lesson: lesson, languages: %w[en ar], disguised: false) }
    before do
  ...
  describe 'disguised_subject_line' do
    ...
    let(:course) { load_course('course-1.json') }
    let(:lesson) { course.lessons.first }
    let(:disguised_mail) { described_class.lesson_email(user: user, course: course, lesson: lesson, languages: %w[en ar], disguised: true) }
